...
★3次以上の項に対応しました。高次の式にはget_hobo()とMIKASAmpler()を使用してください<br>
　➝だいたい5次以上の式だとコンパイルでメモリオーバーすると思います。その場合は [HOBOTAN](https://github.com/ShoyaYasuda/hobotan) の使用を検討してください。
...
## Rust拡張（任意）: GitHubから取得して各自ローカルでビルド
TYTAN には optional な Rust 拡張（`tytan._tytan_rust`）があります。`pip install -U git+...` だけでも動きますが、Rust 拡張を有効化したい場合は **リポジトリを clone して各自の環境でビルド**してください。

前提:
- Rust toolchain（`cargo`）が入っていること
- Python 側は `uv` の `.venv` を使う想定（手元の運用に合わせて読み替え可）

手順（macOS/Linux の例）:
```bash
git clone https://github.com/tytansdk/tytan
cd tytan

# 依存関係（.venv）を用意
uv sync --all-groups

# Rust 拡張を .venv にインストール（開発ビルド）
...
# Rust 拡張が有効か確認
TYTAN_RUST=on TYTAN_RUST_DEBUG=1 uv run python -c "from tytan import _rust_backend as rb; print('rust_available=', rb.rust_available())"
```

補足:
- Rust 拡張は環境変数で制御できます: `TYTAN_RUST=off|auto|on`
- AdaptiveBulkSASampler の「Rust コア経路」は明示的に `TYTAN_RUST_CORE=1` のときだけ有効です（通常は Python 経路の最適化を優先します）。

## テスト
プロジェクトにはユニットテストやインテグレーションテストが含まれており、新しい Adaptive SA 機能も `tests/adaptive_sa` にまとめてあります。各テストファイルは以下のような観点を確認しています。

- `test_reference_sa.py`: CPUベースラインの構造、seed再現性、`return_stats` の出力を検証。
- `test_delta_evaluator.py`: 差分評価が全計算と一致するか、多回反転やランダム問題での安定性。
- `test_solution_pool.py`: best/diverse プールや重複抑制、ハミング距離に基づく多様解取得。
- `test_strategy_manager.py`: ε-greedy による戦略選択、重み更新、複数タイプ対応。
...
Rust 経路のテスト（任意）:
```bash
TYTAN_RUST=on uv run pytest tests/rust
```
...
|日付|ver|内容|
|:---|:---|:---|
|2025/11/28|0.1.4|各samplerの収束力を微調整|
|2025/11/27|0.1.3|各samplerのshots下限値を1に引き下げ（KF様協力）|
